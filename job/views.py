...
#     if not request.user.is_authenticated:
#         return redirect('user_navigation')
#     student = StudentUser.objects.al(user=request.user)
#     s = {'student' : student}
#     return render(request, 'user_navigation.html',s)

def user_navigation(request):
    if not request.user.is_authenticated:
        return redirect('user_navigation')
    student = StudentUser.objects.get(user=request.user)
    s = {'student': student, 'user': request.user}
    return render(request, 'user_navigation.html', s)

# Change profile image view for StudentUser
def change_profile_image(request):
    ...
    if request.method == "POST":
        try:
            ...
            student.image = image
            student.save()
            error = "Profile image updated successfully."
        except Exception as e:
            error = "Error updating image: " + str(e)
    return render(request, 'change_profile_image.html', {'student': student, 'error': error})
    ...
    if request.method == "POST":
        ...
        if user:
            try:
                ...
                if user1.type == "recuiter":
                    if user1.status == "pending":
                        error = "not"  # status is pending
                    elif user1.status.lower() == "rejected" or user1.status.lower() == "reject":
                        error = "rej"  # status is rejected
                    else:
                        login(request, user)
...
def user_home(request):
    if not request.user.is_authenticated:
        return redirect('user_login')
    student = StudentUser.objects.get(user=request.user)
    return render(request,'user_home.html', {'student': student, 'user': request.user})
...
def view_job_user(request):
    ...
    d = {'data': data, 'student': student, 'user': request.user}
...
def applied_condidate_list(request, job_id):
    data=Applyjob.objects.filter(num = job_id)
    d={'data':data}
    return render(request,'applied_condidate_list.html',d)
